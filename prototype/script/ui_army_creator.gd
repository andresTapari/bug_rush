...
#Boton (-) unit_1_delay
func _on_ButtonD_less_pressed()-> void:
		if unit_1_delay > 0:
			unit_1_delay -= 1
			$gd/gdTriangulo/Delay.text=String(unit_1_delay)

#Boton (+) unit_1_delay
func _on_ButtonD_up_pressed()-> void:
		unit_1_delay += 1
		$gd/gdTriangulo/Delay.text=String(unit_1_delay)


#Boton (-) unit_2_delay
func _on_ButtonD_less2_pressed()-> void:
	if unit_2_delay > 0:
		unit_2_delay -= 1
		$gd/gdCubo/Delay.text=String(unit_2_delay)

#Boton (-) unit_2_delay
func _on_ButtonD_up2_pressed()-> void:
	unit_2_delay += 1
	$gd/gdCubo/Delay.text=String(unit_2_delay)
	
#Boton (+) unit_3_delay
func _on_ButtonD_less3_pressed()-> void:
	if unit_3_delay>0:
		unit_3_delay-=1
		$gd/gdEsfera/Delay.text=String(unit_3_delay)

#Boton (+) unit_3_delay
func _on_ButtonD_up3_pressed()-> void:
	unit_3_delay+=1
	$gd/gdEsfera/Delay.text=String(unit_3_delay)


func _on_Button_less_pressed4()-> void:
	if unit_4 > 0:
		recursos += UNIT_STATS.special_unit_type_1.price
		unit_4 -= 1
		$gd/gdCapsula/Label_unit.text=String(unit_4)
		$gd/ProgressBar.value = recursos


func _on_Button_up_pressed4()-> void:
		if recursos >= UNIT_STATS.special_unit_type_1.price:
			recursos -= UNIT_STATS.special_unit_type_1.price
			unit_4 += 1
			$gd/gdCapsula/Label_unit.text=String(unit_4)
			$gd/ProgressBar.value = recursos

func _on_ButtonD_less4_pressed()-> void:
	if unit_4_delay>0:
		unit_4_delay-=1
		$gd/gdCapsula/Delay.text=String(unit_4_delay)


func _on_ButtonD_up4_pressed()-> void:
	unit_4_delay+=1
	$gd/gdCapsula/Delay.text=String(unit_4_delay)
